...
\item[Inductive case] (from 2) \\ If \<open(\meta{s_1}) = close (\meta{s_1})\> \\
...
then \<open(\mconc{(}\meta{s}\mconc{)}) = close(\mconc{(}\meta{s}\mconc{)})\>.
  \begin{itemize}
  \item Induction Hypothesis:  \<open(\meta{s}) = close (\meta{s})\>
  \end{itemize}
